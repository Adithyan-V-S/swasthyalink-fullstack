      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {diseaseRisks && Object.entries(diseaseRisks).map(([disease, risk]) => {
            const riskValue = typeof risk === 'object' ? risk.risk : risk;
            const isMLModel = typeof risk === 'object' && risk.isMLModel;
            ...
            <div key={disease} className="border border-gray-200 rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={`px-2 py-1 rounded text-xs font-medium ${
                  riskValue < 0.2 ? 'bg-green-100 text-green-800' :
                  riskValue < 0.4 ? 'bg-yellow-100 text-yellow-800' :
                  riskValue < 0.7 ? 'bg-orange-100 text-orange-800' :
                  'bg-red-100 text-red-800'
                }`}>
              ...
              <div className="w-full bg-gray-200 rounded-full h-3">
                <div
                  className={`h-3 rounded-full ${
                    riskValue < 0.2 ? 'bg-green-500' :
                    riskValue < 0.4 ? 'bg-yellow-500' :
                    riskValue < 0.7 ? 'bg-orange-500' :
                    'bg-red-500'
                  }`}
              ...
              {/* ML Model specific information */}
              {isMLModel && typeof risk === 'object' && (
                <div className="mt-3 text-xs text-gray-600">
                  <div className="flex justify-between">
                    <span>Confidence: {risk.confidence}</span>
                    <span>Algorithm: {risk.modelInfo?.algorithm}</span>
                  </div>
                  {risk.interpretation && (
                    <p className="mt-1 italic">{risk.interpretation}</p>
                  )}
                </div>
              )}
            </div>
